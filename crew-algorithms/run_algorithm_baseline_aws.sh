...
ALGOS=("CAMON" "COELA" "Embodied" "HMAS_2")

for preset in "${PRESETS[@]}"; do
  for algo in "${ALGOS[@]}"; do
    docker exec -w /app/crew-algorithms/crew_algorithms/wildfire_alg \
      -e PYTHONPATH=/app/crew-algorithms \
      wildfire-algorithm \
      python -m crew_algorithms.wildfire_alg.run_experiments \
        --presets "$preset" \
        --seed ${SEEDS[$preset]} \
        --parallel 5 \
        --algorithm "$algo"
